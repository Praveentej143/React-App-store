...
class AppStore extends Component {
  ...
  getFilterData = () => {
    ...
    const filterRes = appsList.filter(each => each.category === activeTabId)
    return filterRes
  }
  //   getFilterList = searchedApps => {
  //     const {activeTabId} = this.state
  //     const filteredData = searchedApps.filter(
  ...
  render() {
    ...
    const filteredData = this.getFilterData(activeTabId)
    const searchRes = filteredData.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    // const searchResults = this.getSearchResults()
    // const filteredData = this.getFilterList(searchResults)
    ...
    return (
      <div className="app-container">
        <div className="container">
          ...
          {/* app-list goes here */}
          <div>
            <ul className="list-container logo-container">
              {searchRes.map(each => (
                <AppItem appsListComp={each} key={each.appId} />
              ))}
            </ul>
          </div>
        </div>
...
// NOTE : Commented code is also working....its just another method
